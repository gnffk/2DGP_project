...
class AI:
    ...
    def __init__(self):
        self.x = 980
        self.y = 150
        self.weapon_x, self.weapon_y = 850, 330
        self.size_x = 500
        self.size_y = 348
        self.load_images()
    ...
    def draw(self):
        ...
        if self.x - server.hero.x < 100:
            ...
            self.weapon_x = self.x - 130
            self.weapon_y = self.y + 180
    ...
    def ai_attack_time_zero_condition(self):
        if server.hero.state not in 'ATTACK':
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
        pass  # c8
    ...
    def ai_up_attack_time_zero_condition(self):
        probability = random.random()

        # Set the success threshold to 0.7 (70%)
    ...
    def build_behavior_tree(self):
        ...
        c5 = Condition('상대 상태가 IDLE인가?', self.ai_defecne_time_up_condition)
        c6 = Condition('상대 상태가 IDLE이 아닌가', self.ai_defecne_time_zero_condition)
        c7 = Condition('상대방이 공격중이면서 방어 쿨타임이 0인가?', self.hero_attack_condition)
        c8 = Condition('상대가 공격중이 아니거나 나의 방어 쿨타임이 0이 아닌가?', self.ai_attack_time_zero_condition)
        c9 = Condition('ai 중단 쿨타임이 0인가', self.ai_one_attack_time_zero_condition)
        ...
        c17 = Condition('ai 상단공격 쿨타임이 0인가 ', self.ai_up_attack_time_zero_condition)
        ...
        SEQ_UP_ATTACK = Sequence('UP_ATTACK', c17, a5)

        SEQ_UP_ATTACK_OR_IDLE = Selector('UP_ATTACK_OR_IDLE', SEQ_UP_ATTACK, a6)
        SEL_UP_ATTACK_MIDDLE_ATTACK = Selector('UP_ATTACK_MIDDLE_ATTACK', SEQ_MIDDLE_ATTACK, SEQ_UP_ATTACK_OR_IDLE)
        ##

        ##
        SEQ_TWO_ATTACK = Sequence('Two_Attack', c8, a4)
        ...
        SEQ_NOT_BEHAVIOR = Sequence('NOT_BEHAVIOR', c5, a6)
        ##

        ##
        SEL_BEHAVIOR_AND_BACK = Selector('BEHAVIOR_AND_BACK', SEQ_NOT_BEHAVIOR, SEQ_BEHAVIOR)
        ##
        # *
        ...
        # *
        SEL_DISTANCE_LESS_AND_MORE_SCORE_MORE = Selector('DISTANCE_LESS_AND_MORE(SCORE_MORE)',
                                                         SEQ_DISTANCE_LESS_SCORE_MORE, a6)
        ...
        # SEQ_EQUAL = Sequence('EQUAL', c16, a6)#테스트
        # &
        ##
